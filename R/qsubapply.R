...
qsublapply <- function(X, FUN, 
                       qsub.memory="1G", 
                       qsub.tmp.foldername=random::randomStrings(n=1, len=10)[1,], 
                       qsub.src.dir=paste0("/tmp/", qsub.tmp.foldername), 
                       qsub.remote.dir=paste0("/scratch/irc/personal/robrechtc/tmp/", qsub.tmp.foldername)) {
  dir.create(qsub.src.dir)
  mkdir.remote(qsub.remote.dir, remote="irccluster")
  mkdir.remote(paste0(qsub.remote.dir, "/log"), remote="irccluster")
  
  remote.outdir <- paste0(qsub.remote.dir, "/out")
  mkdir.remote(remote.outdir, remote="irccluster")
  
  src.rdata <- paste0(qsub.src.dir, "/data.RData")
  remote.rdata <- paste0(qsub.remote.dir, "/data.RData")
  
  src.rfile <- paste0(qsub.src.dir, "/script.R")
  remote.rfile <- paste0(qsub.remote.dir, "/script.R")
  
  src.shfile <- paste0(qsub.src.dir, "/script.sh")
  remote.shfile <- paste0(qsub.remote.dir, "/script.sh")
  
  save.image(src.rdata)
  
  cp.remote(remote.src="", path.src=src.rdata, remote.dest="irccluster", path.dest=remote.rdata)
...
save(out, file=paste0(\"out/out_\", index, \".RData\", sep=\"\"))")
  ...
  cp.remote(remote.src="", path.src=src.rfile, remote.dest="irccluster", path.dest=remote.rfile)
  
  script <- paste0(
"#!/bin/bash
#$ -t ", 1, "-", length(qsublapply.indices), "
#$ -N interactive
...
Rscript script.R $SGE_TASK_ID")
  ...
  cp.remote(remote.src="", path.src=src.shfile, remote.dest="irccluster", path.dest=remote.shfile)
...
HERE")
  ...
  my.cp.remote <- function (remote.src, path.src, remote.dest, path.dest, verbose = FALSE, 
    ...
    if (recursively) {
      res <- run.remote(paste("scp -r", path.src, path.dest), remote = "")
    } else {
    ...
    if (verbose) 
      print(paste("Elapsed:", attr(res$cmd.out, "elapsed.time"), 
                  "sec"))
    if (via.local) 
      run.remote(paste("rm -f", path.tmp), remote = "")
  }
  
  my.cp.remote(remote.src="irccluster", path.src=remote.outdir, remote.dest="", path.dest=qsub.src.dir, recursively = T)
  
  outs <- lapply(seq_along(qsublapply.indices), function(i) {
